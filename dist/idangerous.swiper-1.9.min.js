...
_this.use3D=_this.support.threeD;var defaults={mode:'horizontal',ratio:1,speed:300,freeMode:false,freeModeFluid:false,slidesPerSlide:1,slidesPerGroup:1,simulateTouch:true,followFinger:true,shortSwipes:true,moveStartThreshold:false,autoPlay:false,onlyExternal:false,createPagination:true,pagination:false,resistance:true,centerSlide:true,scrollContainer:false,preventLinks:true,initialSlide:0,keyboardControl:false,mousewheelControl:false,resizeEvent:'auto',slideElement:'div',slideClass:'swiper-slide',wrapperClass:'swiper-wrapper',paginationClass:'swiper-pagination-switch',paginationActiveClass:'swiper-active-switch'}
...
var wrapper,isHorizontal,slideSize,numOfSlides,wrapperSize,direction,isScrolling,containerSize,centerOffset;for(var i=_this.container.childNodes.length-1;i>=0;i--){if(_this.container.childNodes[i].className){var _wrapperClasses=_this.container.childNodes[i].className.split(' ')
...
wrapperSize=isHorizontal?wrapperWidth:wrapperHeight;centerOffset=(containerSize-slideSize)/2;for(var i=0;i<_this.slides.length;i++){var el=_this.slides[i];if(!_widthFromCSS){el.style.width=slideWidth+"px"
el.style.height=slideHeight+"px"}
if(params.onSlideInitialize){params.onSlideInitialize(_this,el);}}
wrapper.style.width=wrapperWidth+"px";wrapper.style.height=wrapperHeight+"px";if(params.startEndArrows){$(wrapper).append("<div class='arrows'><div class='left-arrow'>&gt;</div><div class='right-arrow'>&lt;</div></div>");}
if(params.initialSlide>0&&params.initialSlide<numOfSlides&&!firstInit){_this.realIndex=_this.activeSlide=params.initialSlide;if(params.loop){_this.activeSlide=_this.realIndex-params.slidesPerSlide}
...
if(params.centerSlide){_this.positions.current+=centerOffset;_this.setTransform(_this.positions.current,0,0);}
if(!firstInit)_this.callPlugins('onFirstInit');else _this.callPlugins('onInit');firstInit=true;}
_this.init()
function maxPos(){var a=(wrapperSize-containerSize);if(params.centerSlide){a+=centerOffset;}
if(params.loop)a=a-containerSize;if(params.scrollContainer){a=slideSize-containerSize;if(a<0)a=0;}
...
if(newPosition==wrapperSize)return;if(curPos>=maxPos()&&!params.loop)return;newPosition-=centerOffset;if(newPosition>maxPos()&&!params.loop){newPosition=maxPos()}
if(params.loop){if(newPosition>=(maxPos()+containerSize))newPosition=maxPos()+containerSize}
...
var groupSize=slideSize*params.slidesPerGroup;var newPosition=(Math.ceil(-getTranslate/groupSize)-1)*groupSize;if(newPosition<0)newPosition=0;if(params.centerSlide){newPosition-=centerOffset;}
if(isHorizontal){_this.setTransform(-newPosition,0,0)}
else{_this.setTransform(0,-newPosition,0)}
_this.setTransition(params.speed)
_this.updateActiveSlide(-newPosition)
...
if(params.centerSlide){newPosition+=(-newPosition+centerOffset)%slideSize;}
...
_this.highlightActiveSlide();if(params.pagination){_this.updatePagination()}}
_this.highlightActiveSlide=function(timeout){if(typeof timeout=='undefined')timeout=500;if(typeof jQuery!='undefined'){for(var i=_this.slides.length-1;i>=0;i--){$(_this.slides[i]).animate({opacity:i==_this.activeSlide?1.0:0.4},timeout);}}}
_this.highlightActiveSlide(0);_this.fixLoop=function(){if(_this.realIndex<params.slidesPerSlide){var newIndex=numOfSlides-params.slidesPerSlide*3+_this.realIndex;_this.swipeTo(newIndex,0,false)}
...
x=x||0;y=y||0;z=z||0;if(this.support.threeD){es.webkitTransform=es.MsTransform=es.msTransform=es.MozTransform=es.OTransform=es.transform='translate3d('+x+'px, '+y+'px, '+z+'px)'}
